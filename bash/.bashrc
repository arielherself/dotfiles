...
run-mount() {
	rclone mount $@: /mnt/$@ --copy-links --allow-other --allow-non-empty --umask 000 --daemon --vfs-cache-mode full --vfs-cache-max-age 2d --vfs-cache-max-size 10G --buffer-size 1G --vfs-read-ahead 500M --vfs-read-chunk-streams 4 --no-modtime -vv
}
run-smount() {
	rclone mount $@: /mnt/$@ --copy-links --allow-other --allow-non-empty --umask 000 --daemon --vfs-cache-mode writes --vfs-cache-max-size 1G --buffer-size 100M --no-modtime --cache-dir $XDG_CACHE_HOME/rclone-cache -vv
}
run-set-mac() {
